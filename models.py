...
class Blstm(object):
    ...
    def numeric_feature_layer(self, hidden_dim, number_features, featureName=None):
        ...
        embeddingLayer = Embedding(number_features, hidden_dim, input_length=self.max_len,
    ...
    def features_layers(self): 
      layers = []
      features_map = feature_to_index(self.features_names, self.scenario)
      ...
      for feature in self.features_names:
        ...
        if feature == 'wordContextFeature':
          layer = self.one_hot_layer(self.max_len, output_len, featureName=feature_name)
        else:
          layer = self.numeric_feature_layer(self.max_len, output_len, self.features_dim, featureName=feature_name)
        layers.append(layer)
